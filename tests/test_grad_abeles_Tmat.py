...
r_ab, t_ab, R_ab, T_ab, A_ab, B_ab = PM.coefficient_with_grad_A(
    chose1, wav, incidence, 0
)
...
for i in range(len(structure) + 1):
    r_ab, t_ab, R_ab, T_ab = PM.coefficient_with_grad_A(
        chose1, wav, incidence, 0, mode="grad", saved_mat=[A_ab, B_ab], i_change=i
    )
    print(f"step {i}: r_ab = {r_ab}")

for i in range(len(structure) + 1):
    r_t, t_t, R_t, T_t = PM.coefficient_with_grad_T(
        chose1, wav, incidence, 0, mode="grad", saved_mat=[A_T, B_T], i_change=i
    )
