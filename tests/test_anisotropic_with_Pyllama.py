...
ani_rot_angle = [0.0, 0.5, -0.2, 0.0]
ani_rot_axis = ["z", "x", [0, 1, 1], "z"]


structure1 = ani.AniStructure(
    material_list, stack, thickness, ani_rot_angle, ani_rot_axis, verbose=False
)
epsilon1 = structure1.rotate_permittivity_tensor(
    np.diag(opt_ind), ani_rot_angle=ani_rot_angle[1], ani_rot_axis=ani_rot_axis[1]
)
epsilon2 = structure1.rotate_permittivity_tensor(
    np.diag(opt_ind), ani_rot_angle=ani_rot_angle[2], ani_rot_axis=ani_rot_axis[2]
)
...
for theta_in_rad in thetas:
    ...
    epsilon_entry = np.array(
        [[n_entry**2, 0, 0], [0, n_entry**2, 0], [0, 0, n_entry**2]]
    )

    epsilon_exit = np.array([[n_exit**2, 0, 0], [0, n_exit**2, 0], [0, 0, n_exit**2]])
    ...
    my_stack_structure = pl.Structure(
        entry, exit, Kx, Ky, Kz_entry, Kz_exit, k0
    )  # at this point my_stack_structure represent only two half-spaces

    my_layer = pl.Layer(epsilon1, thickness_nm, Kx, k0)
    my_stack_structure.add_layer(
        my_layer
    )  # remove_layer and replace_layer() also exist
