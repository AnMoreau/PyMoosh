...
def absorption(struct, wavelength, incidence, polarization):
    ...
    absorb = np.real(tmp[np.arange(0, 2 * g, 2)])
    # reflection coefficient of the whole structure
    r = A[len(A) - 1][0, 0]
    # transmission coefficient of the whole structure
    t = A[len(A) - 1][1, 0]
...
#def Photo(structure,wl_min,wl_max)
...
def Photo(struct,incidence,polarization,wl_min,wl_max,active_layers,number_points):
    ...
    for k in range(number_points):
        absorb = absorption(struct,wavelength_list[k],theta,polarization)
    ...
    total_current_max = np.trapz(wavelength_list,max_current_density)*1e3
    total_current = np.trapz(wavelength_list,current_density)*1e3
    conversion_efficiency = total_current/total_current_max

    return conversion_efficiency,spectrum,total_absorbed
